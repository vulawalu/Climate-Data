...
rfPlt_dry<-ggplot(fijiRF_dry,aes(x=date,y=rainfall_mm,group=site))+
  geom_line(aes(color=site))+
  theme_bw()
...
rfPlt_wet<-ggplot(fijiRF_wet,aes(x=date,y=rainfall_mm,group=site))+
  geom_line(aes(color=site))+
  theme_bw()
...
```
#Attempt to plot average rainfall by dry/wet season months
#I want to see if rain is higher at the beginning of the season or at the end.
#Also if that changes as years go by 2000=2020?
```{r}
#Plot of mean rainfall in dry season
#May-October
  monthlyrain<-fijiRF%>%
  group_by(month,site)%>% #group by year and site and calculate average annual rainfall or mean across years by site
  summarise(aveAnnRF=mean(rainfall_mm,na.rm=T))%>%
  mutate(month=factor(month,levels=c("Jan","Feb","Mar","Apr","May","Jun","Jul","Sug","Sep","Oct","Nov","Dec"))) ## Manually set order of months

monthlyrain%>%
  filter(month %in% c("May", "Jun","Jul", "Aug", "Sep", "Oct"))
  
dry<-ggplot(monthlyrain,aes(x=month,y=aveAnnRF,group=site))+
  geom_line(aes(color=site))+
  theme_bw()
...
wet<-ggplot(monthlyrain,aes(x=month,y=aveAnnRF,group=site))+
  geom_line(aes(color=site))+
  theme_bw()
